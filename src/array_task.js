...
employees.forEach(obj => {
    const e = new Employee(obj.id, obj.name, obj.profileImage, obj.introduction, obj.profileLink)
    newarr.push(e)
})
console.log(newarr)
...
// 1)
const findEmployeeById = (fid) =>{
    let user = newarr.find(emp => emp.id === fid);
    return user;
}

console.log("Employee with specific id", findEmployeeById("8"))



// 2)
so = {name: "Vibhuti Bajaj"}
function findEmployee (obj) {
    const k = Object.keys(obj)[0];
    return newarr.filter((emp) => emp[k] === obj[k] )
}
console.log("Employee with given object", findEmployee(so))




// 3)
function updateEmployeeById(uid, data) {
    k = Object.keys(data)
    v = Object.values(data)
    let idx = newarr.findIndex(emp => emp.id === uid)
    if (k!="id") {
        if (idx !== -1) {
            employees[idx][k] = v[0];
...
console.log("Updated object: ", updateEmployeeById("1", {name:"Vibhuti"}))


// 4)
const deleteBySpecificId = (d_id) =>{
    let res = newarr.filter(emp => emp.id !== d_id)
    return res
}
console.log("After deleting given object", deleteBySpecificId("1"))
